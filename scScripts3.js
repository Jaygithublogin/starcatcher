...
window.onload = function() {
    ...
    var ctx = canvas.getContext("2d"),
        w = canvas.width = 900,
        h = canvas.height = 585;
        ctx.fillStyle= "rgba(150,190,255,.4)";
        ctx.fillRect(50,50,w-125,h-125);
        ctx.fillStyle="red";
        ctx.font="30px Sans-Serif";
        ctx.fillText("Eat Steak, Avoid Salad!",w/3.10,h/2.5);
        ctx.fillText("Best 3 out of 5, Score More Points and Stay Alive to Win",w/15,h/2);
    ...
    // load variables
     var p1x=35, p1y=35, p2x=w-65, p2y=h-65;
     var gameOn=false;
     var p1Score = 0;
     var p2Score = 0;
     var p1Lives=3;
    ...
    for (var i = 0; i < starCount; i++) {
        ...
        starArray.push(star.create(w/2,h/2,5-Math.random()*10,5-Math.random()*10));
    ...
    function starsUpdate () {
        ...
        for (var i = 0; i < starCount; i++) {
            // this checks to see if the star is visible
                ...
                if (Math.abs(p1x-starArray[i]._x)<25 & Math.abs(p1y-starArray[i]._y)<25) {scoring(i,1);}
                if (Math.abs(p2x-starArray[i]._x)<25 & Math.abs(p2y-starArray[i]._y)<25) {scoring(i,2);}
           
                }    
        }//endFor
         //  draw badstar on screen only if visible
        for (var i = 0; i < badStarCount; i++) {
            // this checks to see if the star is visible
                ...
                if (Math.abs(p1x-badStarArray[i]._x)<22.5 & Math.abs(p1y-badStarArray[i]._y)<22.5) {lives(i,1);}
                if (Math.abs(p2x-badStarArray[i]._x)<22.5 & Math.abs(p2y-badStarArray[i]._y)<22.5) {lives(i,2);}
...
// checking for collisions!!!
    ...
    function playerUpdate() {
        ...
        ctx.drawImage(ship1, p1x, p1y, 45, 45);
        ctx.drawImage(ship2, p2x, p2y, 45, 45);
    }   
...
function lives(k,wp) {
    ...
    function endGame(wp) {
        ctx.fillStyle= "rgba(250,200,0,.4)";
        ctx.fillRect(50,50,w-100,h-100);
        ctx.fillStyle="white";
        ctx.font="30px Sans-Serif";
        if (wp==1){
            ctx.fillText("Game Over! Shrek Wins!",w/3,h/2);
        }
        if (wp==2){
            ctx.fillText("Game Over! Mouth Wins!",w/3,h/2);
        }       
    }  // close endGame
